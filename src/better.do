
* Loop over crops

    cd = "~\Dropbox-temp11\project"

    global myGlobal = "A string global"
    display "${myGlobal}"
    display "$myGlobal"

    // some comment

    forvalues ii in potato potato cassava maize potato cassava maize potato cassava maize ///
        potato cassava maize potato cassava maize potato cassava maize ///
        potato cassava maize potato cassava maize { 
        do something to `i' 
        if something ~= 1 & something != . {
            do something  // some very very very very very very very very very very very very very very very very very very very very very very long comment
        } 
        else if something something something something something something ///
            something something something something something {  // some comment some comment some comment some comment some comment some comment

            do something 
        }
    }

    if something something something something something something ///
        something something {
        do something
    }

    if missing(something) do something
    local a = "asodfas as[odifgaj sdkjf  asodif asl;kdjf a;sdigfuas ;dlfkjasda;sldiufasodfas as[odifgaj sdkjf  asodif asl;kdjf a;sdigfuas ;dlfkjasda;sldiuf" ///
        asodfas as[odifgaj sdkjf  asodif asl;kdjf a;sdigfuas ;dlfkjasda;sldiufasodfas ///
        as[odifgaj sdkjf  asodif asl;kdjf a;sdigfuas ;dlfkjasda;sldiuf
    di `a'

    if something something something something something something ///
        something something { // some comment
        do something 
    } 

