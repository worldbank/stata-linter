...
preserve
	...
	by D37, sort: eststo: estpost summarize female ageHH finprim cattlewealth bornvillage VHethnicmatch VHfam diffview
	esttab using "${tables}/table_c1", tex replace cells("mean(fmt(a3))" sd(par fmt(a3))) label nodepvar noobs onecell nonotes
	esttab using "${tables}/table_c1_new.tex", replace cells("mean(fmt(a3))" sd(par fmt(a3))) ${stars1}
	eststo clear
...
preserve
	...
	// Born Village
	eststo clear 
	tvsc `vars' if VH_treat == 0, by(bornvillage) clus_id(male) strat_id(male)	
...
eststo clear
loc indexlist communal numhouseholds numbergroups womenVH moreprimary age tenure VH_bornvillage logcattlewealth ZANUPF_para
loc n 0
...
eststo clear
loc indexlist2 female finprim ageHH bornvillage wagelabor VHfam VHethnicmatch logcattlewealth diffview
loc n 0
...
// Proposed changes
eststo clear
eststo e1: evccl_diff biasindex if diffview == 1, by(VH_treat) clus_id(cc) diff(diffview)
...
// Proposed changes
eststo clear
eststo e1: evccl_diff biasindex food_aid mostfair if diffview == 1, by(VH_treat) clus_id(cc) diff(diffview)
esttab e1 using "${tables}/TableH3a_new.tex",  replace ${stars1}		///
	cells("mu_0(fmt(%9.3fc)) mu_1(fmt(%9.3fc)) mu_3(fmt(%9.3fc)) mu_2(fmt(%9.3fc)) N_S(fmt(%9.0fc))" "se_0(par) se_1(par) se_3(par) se_2(par)") 	

eststo e2: evccl resolvedindex nofoodaidneed noconflicts, by(VH_treat) clus_id(cc)
...
// Proposed changes
eststo clear
evccl_diff testknowledge testattitudes st_proceduresindex if closehat_di == 0, by(VH_treat)
...
foreach index of loc indexlist2{
	loc ++n
	reg `index' VH_treat cl i.block if closehat_di==0 & diffview==1, cluster(cc)
...
// Proposed changes
eststo clear
evccl_diff biasindex if closehat_di == 0 & diffview == 1, by(VH_treat) clus_id(cc)
...
// Proposed changes
eststo clear
evccl_diff testknowledge testattitudes st_proceduresindex if closehat_di == 1, by(VH_treat)
...
foreach index of loc indexlist2{
	loc ++n
	reg `index' VH_treat cl i.block if closehat_di==1 & diffview==1, cluster(cc)
...
// Proposed changes
eststo clear
evccl_diff biasindex if closehat_di == 1 & diffview == 1, by(VH_treat) clus_id(cc)
...
loc indexlist st_proceduresindex percwomendare consult_women consult_RMC transparency nofee
loc n 0
...
// Proposed changes
eststo clear
evccl st_proceduresindex percwomendare consult_women consult_RMC transparency nofee, by(VH_treat) 
...
foreach index of loc indexlist{
 ...
 // Proposed changes
eststo clear
evccl st_legitimacyindex trust relationship compliance livestocktoVH, by(VH_treat) 
...
outreg2 using "${tables}/tableI4", replace tex(frag) se label nonotes noaster nor2 nocons

// 2nd column
local balance female finprim bornvillage wagelabor VHfam VHethnicmatch ageHH logcattlewealth
...
outreg2 using "${tables}/tableI4", append tex(frag) drop(`balance') se label nonotes noaster nor2 nocons
...
esttab reg1 reg2 using "${tables}/tableI4_new.tex", replace keep(VH_treat cl `vlist') ${stars3} scalars("p_diff P-value from T-test")
...
// Regression
eststo clear 
eststo reg1: logit close vehicles notfarminginc moreprimary parentsherewhenborn if cl==1
estat classification, cutoff(0.35)
estadd scalar corr = r(P_corr)

// Export
esttab using "${tables}/tableJ1_new.tex", replace ${stars3} scalars("corr PCC (0.35 cutoff)")
...
esttab using "${tables}/TableK2a", tex replace keep(cl) label nostar nogaps nonotes b("%9.3f") se("%9.3f")

eststo clear
by moreprimary, sort: eststo: regress st_proceduresindex VH_treat cl i.block
esttab using "${tables}/TableK2b", tex replace keep(cl) label nostar nogaps nonotes b("%9.3f") se("%9.3f")

use "${data}/HHdata_QJPS_labeled.dta", clear

eststo clear
by moreprimary, sort: eststo: regress biasindex VH_treat cl i.block if diffview==1, cluster(cc)
esttab using "${tables}/TableK2c", tex replace keep(cl) nostar label nogaps nonotes b("%9.3f") se("%9.3f")

eststo clear
by moreprimary, sort: eststo: regress st_legitimacyindex VH_treat cl i.block, cluster(cc)
esttab using "${tables}/TableK2d", tex replace keep(cl) nostar label nogaps nonotes b("%9.3f") se("%9.3f")
...
esttab using "${tables}/TableK3a", tex replace keep(cl) nostar label nogaps nonotes b("%9.3f") se("%9.3f")

eststo clear
by ccount, sort: eststo: regress st_proceduresindex VH_treat cl i.block
esttab using "${tables}/TableK3b", tex replace keep(cl) nostar label nogaps nonotes b("%9.3f") se("%9.3f")

use "${data}/HHdata_QJPS_labeled.dta", clear

eststo clear
by ccount, sort: eststo: regress biasindex VH_treat cl i.block if diffview==1, cluster(cc)
esttab using "${tables}/TableK3c", tex replace keep(cl) nostar label nogaps nonotes b("%9.3f") se("%9.3f")

eststo clear
by ccount, sort: eststo: regress st_legitimacyindex VH_treat cl i.block, cluster(cc)
esttab using "${tables}/TableK3d", tex replace keep(cl) nostar label nogaps nonotes b("%9.3f") se("%9.3f")
...
esttab using "${tables}/TableK4a", tex replace keep(cl) nostar nogaps label nonotes b("%9.3f") se("%9.3f")

eststo clear
by diffview, sort: eststo: regress st_legitimacyindex VH_treat cl i.block
esttab using "${tables}/TableK4b", tex replace keep(cl) nostar nogaps label nonotes b("%9.3f") se("%9.3f")
